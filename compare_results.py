...
Loads saved results from both classifiers (RF, CNN) and both pH thresholds
(7.05, 7.15) and produces:
  1. A summary table (printed + saved as CSV).
  2. A combined ROC curve with all 4 variants.
...
def build_summary_table() -> pd.DataFrame:
    ...
    for threshold in THRESHOLDS:
        ...
        for clf_name in CLASSIFIERS:
            ...
            # CV metrics
            if clf_name == "rf":
                cv = _load_rf_cv(suffix)
            else:
                cv = _load_cnn_cv(suffix)
...
def plot_combined_roc():
    ...
    ax.set_title("Combined ROC Curves — All Classifiers & Thresholds", fontsize=13)
...
def main():
    print("=" * 70)
    print("COMPARISON — CTG Classifier Validation Results")
    print("=" * 70)
